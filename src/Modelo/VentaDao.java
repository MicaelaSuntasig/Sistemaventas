...
public class VentaDao {
    ...
    public List Listarventas(){
       ...
       String sql = "SELECT c.dni AS id_cli, c.dni, v.* FROM clientes c INNER JOIN ventas v ON c.dni = v.cliente";
       try {
           con = cn.getConnection();
           ps = con.prepareStatement(sql);
           rs = ps.executeQuery();
           while (rs.next()) {               
               ...
               vent.setId(rs.getString("id_cli"));
               vent.setIn(rs.getInt("id"));
               vent.setNombre_cli(rs.getString("dni"));
    ...
    public void pdfV(int idventa, String Cliente, double total, String usuario) {
        try {
            ...
            doc.open();
            Image img = Image.getInstance(getClass().getResource("/Img/logo inicio.png"));
            //Fecha
            ...
            Encabezado.addCell("");
            //info empresa
            String config = "SELECT * FROM empresa";
            String mensaje = "";
            try {
                con = cn.getConnection();
            ...
            String prove = "SELECT * FROM clientes WHERE dni = ?";
            try {
            ...
            tabla.addCell(c3);
            tabla.addCell(c4);
            String product = "SELECT d.id, d.id_pro,d.id_venta, d.precio, d.cantidad, p.codigo, p.nombre FROM detalle d INNER JOIN productos p ON d.id_pro = p.codigo WHERE d.id_venta = ?";
            try {
